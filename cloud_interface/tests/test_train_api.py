...
class TestTestDataStorage(unittest.TestCase):
    ...
    def test_generate_name(self):
        ...
        # Assert
        self.assertTrue(result[0].startswith('train_data_'))
        self.assertTrue(result[0].endswith('.zip'))
        self.assertTrue(result[1].startswith('train_data_'))
        self.assertEqual(len(result[1]), 25)  # Length of timestamp string (YYYYMMDDHHMMSS)
